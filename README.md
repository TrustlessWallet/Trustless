...
6. Locate the signing tool and sign the package using the keystore: 
   ...
   `$apksigner_path sign --ks trustless-release.keystore --ks-key-alias trustless-alias --out trustless-v1.2.0-release.apk android/app/build/outputs/apk/release/app-release-unsigned.apk`
7. Generate the official hash: 
   `shasum -a 256 trustless-v1.2.0-release.apk`
...
1.  **Download the signed release:**
    ...
    ```bash
    curl -L -o trustless-release.apk https://github.com/trustlesswallet/trustless/releases/download/1.2.0/trustless-v1.2.0-release.apk
    ```
